...
def unit_propagation(valuation, clauses):
    ...
    * Otherwise, if all literals of a clause are False, then the whole clause
    ...
    """
    """def at_least_one(valuation, clause):
        for s,t in clause:
            if s in valuation and valuation[s] == t:
                return True
        return False"""

    def not_det(valuation, clause):
        n = 0
        for s,t in clause:
            if s not in valuation.keys():
                n += 1
                if n > 1:
                    return True
                else:
    ...
    def all_false(valuation, clause):
        n = 0
        for s,t in clause:
            if s in valuation.keys() and valuation[s] != t:
                n += 1
                if n == len(clause):
                    return True
        else:
    ...
    while clauses2:
        ...
        for clause in clauses2:
            non = []
            # if all false
            if all_false(valuation, clause):
                return (False, {})
            # if some clause has more than one non determined atom
            elif not_det(valuation, clause):
                notdet += 1
            else:
                no_true_values = True
                for s,t in clause:
                    if s not in valuation:
                        non.append((s,t)) 
                    elif s in valuation and valuation[s] == t:
                        no_true_values = False
                        break
                # if some clause has no true values and others all false + one not determined
                if len(non) == 1 and no_true_values:
                    unit_c.append(clause)
                    valuation[non[0][0]] = non[0][1]

        if not unit_c and notdet > 0:
            return (True, {})    
        elif not unit_c:
...
def dpll(valuation, clauses, variables):
    ...
    (satisfiability, val_1) = unit_propagation(valuation, clauses)
    ...
    valuation2 = val_1.copy()
    val_1[branching_variable] = True
    result1,res1 = dpll(val_1, clauses, variables)

    if result1:
        valuation.update(res1)
        return (True, valuation)

    else:
        valuation2[branching_variable] = False
        result2, res2 = dpll(valuation2, clauses, variables)

        if result2:
            valuation.update(res2)
            return (True, valuation)
        else:
            return (False, {})
