...
class MyTestCase(unittest.TestCase):
    def check(self, formula):
        ...
        z3_result, hi, _ = solve(formula)
        sat_to_string = {True: 'Satisfiable', False: 'Unsatisfiable'}
        self.assertEqual(z3_result, my_result, "\nmy: {}, {},\n{} {} \nExpected value: {}, Actual value: {}".format(my_result, model,z3_result, hi,sat_to_string[z3_result],
                                                                                             sat_to_string[my_result]))
        if my_result:
            self.assertTrue(formula.is_satisfiable(model), "{},{} Valuation is not correct".format(formula.clauses(),sat_to_string[z3_result]))
        return my_result
